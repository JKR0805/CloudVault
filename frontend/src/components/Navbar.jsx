  ...
  return (
    <nav className="glass-panel navbar" style={{ 
      ...
      <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '1.25rem', fontWeight: 'bold' }}>
        ...
        <span className="navbar-brand-text">CloudVault</span>
      </Link>
      ...
      {user && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            />
            <span className="navbar-user-name" style={{ fontWeight: '500' }}>{user.user_metadata?.full_name || user.email}</span>
          </div>
          <button onClick={logout} className="btn btn-outline" style={{ padding: '8px 12px' }}>
            <LogOut size={16} /> <span className="navbar-logout-text">Logout</span>
          </button>
        </div>
      )}
    </nav>
